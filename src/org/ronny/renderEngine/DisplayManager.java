package org.ronny.renderEngine;

import org.lwjgl.LWJGLException;
import org.lwjgl.opengl.ContextAttribs;
import org.lwjgl.opengl.Display;
import org.lwjgl.opengl.DisplayMode;
import org.lwjgl.opengl.GL11;
import org.lwjgl.opengl.PixelFormat;

public class DisplayManager {
	
	private static final String TITLE = "My Raw Model";
	private static int WIDTH = 1280, HEIGHT = 720;
	private static final int FPS_CAP = 120;
	
	public static void createDisplay() {
		// attempt to get user screen size. 
		//		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		//		WIDTH = (int) (screenSize.getWidth() * 0.9);
		//		HEIGHT = (int) (screenSize.getHeight() * 0.9);
		
		ContextAttribs contextAttributes = new ContextAttribs(3, 2).withForwardCompatible(true).withProfileCore(true);
		try { // initialize openGL settings and display mode.
			Display.setDisplayMode(new DisplayMode(WIDTH, HEIGHT));
			Display.create(new PixelFormat(), contextAttributes);
			Display.setTitle(TITLE);
		} catch (LWJGLException e) {
			e.printStackTrace();
		}
		
		// where openGL can render the game.
		GL11.glViewport(0, 0, WIDTH, HEIGHT);
	}
	
	public static void updateDisplay() {
		Display.sync(FPS_CAP);
		Display.update();
	}
	
	public static void closeDisplay() {
		Display.destroy();
	}
	
}
